...
export class ClaudeCodeHookAdapter {
  ...
  constructor(private readonly options: ClaudeCodeHookAdapterOptions = {}) {}
  ...
  ingest(hook: unknown): AgentTraceEvent[] {
    ...
    if (!this.opened && eventName !== "UserPromptSubmit") {
      ...
      return [
        ...this.openRun(sessionId, "(session)"),
        ...this.ingest(hook),
      ];
    }
    ...
    switch (eventName) {
      ...
      case "PreToolUse":
        ...
        break;
      case "PostToolUse":
        handled = [...context, ...this.onPostTool(msg, false)];
        break;
      case "PostToolUseFailure":
        handled = [...context, ...this.onPostTool(msg, true)];
        break;
      case "SubagentStart":
    ...
    return [...handled, ...this.noteTranscript(msg)];
  }

  private noteTranscript(msg: Record<string, unknown>): AgentTraceEvent[] {
    if (!this.opened) return [];
    const path = asString(msg.transcript_path);
    if (!path) return [];
    const read = this.options.readTranscript ?? readTranscriptFile;
    const text = read(path);
    if (!text) return [];
    const usage = usageFromTranscriptText(text);
    const model = modelFromTranscriptText(text);
    let changed = false;
    if (usage) {
      this.usage = usage;
      changed = true;
  ...
  private onPostTool(msg: Record<string, unknown>, failed: boolean): AgentTraceEvent[] {
