...
<head>
  ...
  <title>Claude Code · agent-think-map</title>
...
<body>
  <div class="studio">
    <aside class="rail">
      <h1>Sessions</h1>
      <p>Live map for this Claude Code CLI. Click a session.</p>
      ...
      <div id="sessions" class="empty">Waiting for Claude Code…</div>
    </aside>
  ...
  <script>
    ...
    function metaLine(session) {
      ...
      if (session.model) parts.push(String(session.model).replace(/^claude-/, ""));
    ...
    function draw() {
      ...
      if (!allSessions.length) {
        ...
        list.textContent = "Waiting for Claude Code…";
        return;
      }
      if (!sessions.length) {
        list.className = "empty";
