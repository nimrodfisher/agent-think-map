...
for(const [Hub,create] of [[CodexTraceHub,createCodexStudio],[ClaudeCodeTraceHub,createClaudeCodeStudio]] as const)it(Hub.name+' diff API validates sides, persists results and rejects foreign Host/Origin',async()=>{
 ...
 try {
  const pair={badRunId:'bad',goodRunId:'good'};const response=await post(pair);expect(response.status).toBe(200);const diff=await response.json();expect(diff).toMatchObject(pair);expect(await (await fetch(base+'/api/diffs/'+diff.diffId)).json()).toEqual(diff);
