...
it('substitutes input shape for same operation',()=>expect(align([{name:'read',input:{path:'a.ts'}}],[{name:'read',input:{path:'a.py'}}]).map(r=>r.classification)).toEqual(['matched','changed']));
...
it('never matches across user turns',()=>{const rows=align([{name:'read'},{name:'bash',turn:1}],[{name:'bash'},{name:'read',turn:1}]);expect(rows.filter(r=>r.classification==='matched')).toHaveLength(2);});
